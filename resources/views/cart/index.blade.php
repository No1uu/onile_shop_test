...
@section('content')
    ...
    <div class="grid lg:grid-cols-3 lg:gap-x-8 gap-x-6 gap-y-8 mt-6">
        <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="ml-auto flex flex-col">
                            <div class="flex items-start gap-4 justify-end">
                              <!-- <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 cursor-pointer fill-slate-400 hover:fill-pink-600 inline-block" viewBox="0 0 64 64">
                                ...
                                <form action="{{route('cart.remove')}}" method="post" onsubmit="return ('Are you sure?')">
                                    @csrf
            ...
            @empty
                <div class="bg-white px-4 py-6 rounded-md shadow-sm border border-gray-200 text-center">
                    <p class="text-center text-gray-500">Таны хоосон байна</p>
                </div>
            @endforelse
                </div>
                ...
                <div class="bg-white rounded-md px-4 py-6 h-max shadow-sm border border-gray-200">
                    <ul class="text-slate-500 font-medium space-y-4">
                        <li class="flex flex-wrap gap-4 text-sm">Price <span class="ml-auto font-semibold text-slate-900">{{number_format($subtotal)}}₮</span></li>
                        ...
                        <li class="flex flex-wrap gap-4 text-sm font-semibold text-slate-900 subtotal">Нийт үнийн дүн<span class="ml-auto">{{ number_format($subtotal) }}₮</span></li>
                    </ul>
                    <div class="mt-8 space-y-4">
                        
                        <a type="button" href="{{route('order.save')}}" class="block text-center text-sm px-4 py-2.5 w-full font-medium tracking-wide bg-slate-800 hover:bg-slate-900 text-white rounded-md cursor-pointer">Худалдаж авах</a>
                        <form action="{{route('cart.clear')}}" method="post" onsubmit="return confrim('Are you sure?')">
                            @csrf
